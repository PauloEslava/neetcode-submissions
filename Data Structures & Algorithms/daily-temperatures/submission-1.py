class Solution:
    def dailyTemperatures(self, temperatures: List[int]) -> List[int]:
        ...
        for i in range(0, len(temperatures)):
            ...
            # Here we verify isnt empty at first
            if tempStack:
                # This one also check for tempStack because since we are emptying it with pop(),
                # we have to make sure that it is never empty for every iteration, otherwise [-1] breaks
